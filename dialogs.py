try:
    from qt.core import (QWidget, QVBoxLayout, QLabel, QLineEdit, QPushButton, 
                         QFileDialog, QDialog, QListWidget, QHBoxLayout, QMessageBox, 
                         QTableWidget, QTableWidgetItem, QHeaderView, QAbstractItemView, 
                         QInputDialog, QListWidgetItem, Qt, QFont, QCheckBox, QGroupBox)
except ImportError:
...
from calibre_plugins.epub_template_master.config import (
    get_templates, get_default_template, set_default_template,
    add_template, remove_template, get_template_path, get_template_dir,
    get_duplicate_all_formats, set_duplicate_all_formats
)
from calibre_plugins.epub_template_master.logic import (
    import_template_file, delete_template_file, rename_template_file
)
import os
...
class ConfigWidget(QWidget):
    ...
    def import_templates(self):
        paths, _ = QFileDialog.getOpenFileNames(self, "选择 EPUB 文件", "", "EPUB Files (*.epub)")
        if not paths:
            return
    ...
    def rename_template(self):
        ...
        templates = get_templates()
        current_name = next((t['name'] for t in templates if t['filename'] == filename), filename)
    ...
    def edit_note(self):
        ...
        filename = filenames[0]
        templates = get_templates()
        current_note = next((t.get('note', '') for t in templates if t['filename'] == filename), '')
        
        new_note, ok = QInputDialog.getText(self, "编辑备注", "备注:", text=current_note)
        if ok:
            for t in templates:
    ...
    def delete_templates(self):
        ...
        if QMessageBox.question(self, "确认", 
                f"删除 {len(filenames)} 个模板?") == QMessageBox.Yes:
            count = 0
...
class MainActionDialog(QDialog):
    ...
    def __init__(self, parent=None, has_selection=False):
        ...
        create_settings_btn.clicked.connect(lambda: self.select_template_and_do(self.ACTION_CREATE))
        row1.addWidget(create_settings_btn)
        layout.addLayout(row1)
        
        # 插入书籍
        row2 = QHBoxLayout()
        insert_btn = QPushButton('2. 插入书籍', self)
        ...
        insert_settings_btn.clicked.connect(lambda: self.select_template_and_do(self.ACTION_INSERT))
